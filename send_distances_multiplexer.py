...
# Sensor + multiplexer setup
bus = smbus2.SMBus(1)
MULTIPLEXER_ADDR = 0x70  # TCA9548A default address
MULTIPLEXER_CHANNELS = [0, 1, 2, 3, 4]  # One for each TF-Luna sensor
SENSOR_ADDR = 0x10  # All sensors share this I2C address

def select_mux_channel(channel):
    if 0 <= channel <= 7:
        bus.write_byte(MULTIPLEXER_ADDR, 1 << channel)
        time.sleep(0.01)  # Small delay to allow switching
    else:
...
class DistanceCharacteristic(dbus.service.Object):
    ...
    def update_sensor_value(self):
        readings = []
        for ch in MULTIPLEXER_CHANNELS:
            try:
                ...
                print(f"Error reading on channel {ch}: {e}")
                readings.append("ERR")
