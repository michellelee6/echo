...
def ssd_postprocess(outputs, threshold):
    ...
    boxes = boxes[0]
    class_scores = classes[0]  # shape: [num_detections, num_classes]
    scores = scores[0]
    num = int(num_detections[0][0])
    results = []
    for i in range(num):
        if scores[i] >= threshold:
            cls = int(np.argmax(class_scores[i]))  # Fix: use argmax for class index
            results.append((boxes[i], scores[i], cls))
    return results
